...
#from MainUI.login import LoginPage
...
class User(tk.Frame):
    ...
    def __init__(self, root):
        ...
        self.age = tk.Entry(self, width=40)
        ...
        l_userName = tk.Label(self, text="UserName ")
        l_password = tk.Label(self, text="Password ")
        l_age = tk.Label(self, text="Age ")

        # Title label for page
        title = tk.Label(self, text="User Registration", fg="black", font=("times new roman", int(20.0)), pady=10)

        # Button to add member to Dtbf.create(self)
        create = tk.Button(self, text="Create", command=self.create, width=20)
        back = tk.Button(self, text="Back",  width=5, command=lambda: root.change_page(LoginPage))
        ...
        l_userName.grid(row=6, column=0, pady=10)
        l_password.grid(row=7, column=0, pady=10)
        l_age.grid(row=8, column=0, pady=10)
        ...
        self.age.grid(row=8, column=1, padx=5)

        create.grid(row=9, column=1, columnspan=2, pady=15)
        back.grid(row=9, column=0, padx=5, pady=15, sticky="ns")
    ...
    def create(self):
        ...
        if not aget:
            tk.messagebox.showerror(
                title="Empty Fields!", message="Please enter age.")
            return
        # self.generate_id()
        ...
        if [x for x in (self.first_name, self.last_name, self.email, self.address, self.number) if x is None]:
            ...
            d = UserDB()
            d.add_User(self.first_name.get(), self.last_name.get(), self.email.get(), self.address.get(),
                                self.number.get(), self.userName.get(), self.userName.get(), self.age.get())
    ...
    def validate_age(self):
            if len(self.age.get()):
                return False
            elif self.number_regex.search(self.age.get()):
                return False
            else:
                return True
