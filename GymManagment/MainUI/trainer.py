...
class Trainer(tk.Frame):
    ...
    def __init__(self, root):
        ...
        self.specialist = tk.Entry(self, width=40)
        ...
        l_specialist = tk.Label(self, text="Specialist ")

        # Title label for page
        title = tk.Label(self, text="Create A Trainer", fg="black", font=("times new roman", int(20.0)), pady=10)
        ...
        l_specialist.grid(row=6, column=0, pady=10)
        ...
        self.specialist.grid(row=6, column=1, padx=5)

        create.grid(row=7, column=1, columnspan=2, pady=15)
        back.grid(row=7, column=0, padx=5, pady=15, sticky="ns")
    ...
    def create(self):
        ...
        specialist = self.specialist.get()
        if not fName:
        ...
        if not specialist:
            tk.messagebox.showerror(
                title="Empty Fields!", message="Please enter Specialist.")
            return
        # self.generate_id()
        ...
        if [x for x in (self.first_name, self.last_name, self.email, self.address, self.number) if x is None]:
            ...
            d = TrainerDB()
            d.create_trainer(self.first_name.get(), self.last_name.get(), self.email.get(), self.address.get(),
                             self.number.get(), self.specialist.get())
            ...
            self.specialist.delete(0, tk.END)

            showinfo("Added", "Successfully Added Trainer")
